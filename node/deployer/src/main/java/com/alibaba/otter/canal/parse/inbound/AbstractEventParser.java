...
public abstract class AbstractEventParser<EVENT> extends AbstractCanalLifeCycle implements CanalEventParser<EVENT> {
    ...
    // 统计参数
    protected AtomicBoolean profilingEnabled = new AtomicBoolean(true);                // profile开关参数
    ...
    //并发解析binlog
    protected ExecutorService parseService = Executors.newFixedThreadPool(50);
                    ...
                    try {
                        ...
                        final SinkFunction sinkHandler = new SinkFunction<EVENT>() {
                            ...
                            public boolean sink(EVENT event) {
                                try {
                                    return sinkEntry(parseAndProfilingIfNecessary(event,false));
                                }catch (Exception e){
                            ...
                            @Override
                            public List<Entry> parse(List<EVENT> events, int batchCount) {
                                try {
                                    return parseEvent(events,batchCount);
                                }catch (Exception e){
                                    throw new RuntimeException(e.getMessage());
                                }
                            }
    ...
    private class ParseThread implements Callable {
        List<EVENT> events = null;

        public ParseThread(List<EVENT> events) {
            this.events = events;
        }

        public List<Entry> call() throws Exception {
            List<CanalEntry.Entry> entries = new ArrayList<Entry>();
            for (EVENT event : events) {
                entries.add(binlogParser.parse(event, false));
            }
            return entries;
        }
    }
    ...
    protected List<CanalEntry.Entry> parseEvent(List<EVENT> events, int batchCount) throws Exception {
        ...
        int i = 1;
        for (; i < batchCount; i++) {
            ParseThread thread = new ParseThread(events.subList((i - 1) * batchSize, i * batchSize));
            futures.add(parseService.submit(thread));
        }
        ParseThread thread = new ParseThread(events.subList((i - 1) * batchSize, events.size()));
        futures.add(parseService.submit(thread));
        List<CanalEntry.Entry> entries = new ArrayList<Entry>();
