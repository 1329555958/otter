...
public class MemoryEventStoreWithBuffer extends AbstractCanalStoreScavenge implements CanalEventStore<Event>, CanalStoreScavenge {
    ...
    final Logger LOG = LoggerFactory.getLogger("com.wch.test");
    ...
    public boolean tryPut(List<Event> data) throws CanalStoreException {
        ...
        if (System.currentTimeMillis() - LOG_TIME > 3000 || EVENT_COUNT % 10000 == 0) {
            ...
            LOG_TIME = System.currentTimeMillis();
            TOTAL += EVENT_COUNT;
            LOG.info("3333333333 events now is {}, new {} now is [file={},pos={},time={}]", TOTAL, EVENT_COUNT, header.getLogfileName(), header.getLogfileOffset(), TIME_FORMAT.format(new Date(header.getExecuteTime())));
            EVENT_COUNT = 0;
        }
        if (PUT_TIME == 0) {
        ...
        PUT_TIME = 0;
        CanalEntry.Header header = data.get(data.size() - 1).getEntry().getHeader();
        LOG.info("4444444444 now put {} events,last is [file={},pos={},time={}]", data.size(), header.getLogfileName(), header.getLogfileOffset(), TIME_FORMAT.format(new Date(header.getExecuteTime())));
        final ReentrantLock lock = this.lock;
